...
function ipmiJobFactory(
    ...
    IpmiJob.prototype._run = function run() {
        ...
        var self = this;
           return waterline.workitems.update({name: "Pollers.IPMI"}, {failureCount: 2})
        .then(function() {
            ...
            self._subscribeRunIpmiCommand(self.routingKey, 'powerStatus',
                self.createCallback('powerStatus', self.collectIpmiChassisPowerStatus.bind(self)));
        })
        .catch(function(err) {
     ...
     */
     IpmiJob.prototype.powerStateAlerter = Promise.method(function(status, data) {
        ...
        console.log('test ' + self.cachedPowerState[data.workItemId] +'\t'+ status.status);
        if(self.cachedPowerState[data.workItemId] !== status.status) {
            console.log('power status changed from ' + self.cachedPowerState[data.workItemId] + ' to ' + status.status)
            self._publishPollerAlert(tmp);
            self.cachedPowerState[data.workItemId] = status.status;
        }
        console.log('pink');
        console.log(status);
        return status;
    ...
    /**
     *Collect chassis power status data from IPMI
     *@memberOf IpmiJob
     *
     *@param data
     */
    IpmiJob.prototype.collectIpmiChassisPowerStatus = function(data) {
        console.log('apple');
        var self = this;
        return ipmitool.chassisPowerStatus(data.host, data.user, data.password)
        .then(function(status) {
            return parser.parseChassisPowerStatus(status);
        });
        //.spread(self.powerStateAlerter.bind(self))
        //.then(function(status) {
        //    return status;
