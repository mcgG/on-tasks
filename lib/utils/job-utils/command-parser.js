...
function commandParserFactory(Logger, Promise, _) {
    ...
    CommandParser.prototype[ls] = function(data) {
        if (data.err) {
            return Promise.resolve({ source: 'ls -l /var', error: data.error });  
        }
        try {
            var parsed = [];
            data.split('\n').forEach(function(line) {
                ...
                if (fields.length > 9) {
                    fields = fields.slice(0, 5)
                                .concat(fields.slice(5, 8).join(' '))
                                .concat(fields.slice(8, fields.length).join(''));
                } else {
                    return;
                }
                parsed.push({
                    'file_type': fields[0][0] === '-' ? 'file' : fields[0][0] === 'd' ? 'directory' : 'link',
                    'permission': {
                        "user": fields[0].slice(1, 4),
                        "group": fields[0].slice(4, 7),
                        "others": fields[0].slice(7, 10)
                    },
                    "link_count": fields[1],
                    "owner": fields[2],
                    "group": fields[3],
            ...
            return Promise.resolve({ data: parsed, source: 'ls -l /var', store: true });    
        } catch (e) {
            return Promise.resolve({ source: 'ls -l /var', error: e });
        }
    }
